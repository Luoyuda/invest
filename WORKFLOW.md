...
- `scripts/fetch_a_share_data.py`：通过 iFinD MCP 获取行情；交互式 agent 优先用 iFinD skill。
- `scripts/fetch_capital_flow.py`：通过 iFinD MCP 获取资金流；交互式 agent 优先用 iFinD skill。
- `scripts/fetch_sector_boards.py`：通过 iFinD MCP 获取板块快照；交互式 agent 优先用 iFinD skill。
...
- `scripts/search_news.py`：通过 iFinD MCP 获取公告资讯；交互式 agent 优先用 iFinD skill。
