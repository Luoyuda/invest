...
grep -q 'iFinD skill' references/a-share-data-sources.md || fail "data sources must document iFinD skill priority"
grep -q 'iFinD MCP' references/a-share-data-sources.md || fail "data sources must document iFinD MCP fallback"
grep -q 'ifind' references/a-share-data-sources.md || fail "data sources must document ifind provider"
grep -q 'iFinD skill / iFinD MCP' references/capital-flow.md || fail "capital flow reference must document iFinD provider"
...
grep -q 'runtime/recommendation-runs/<YYYY-MM-DD>/<session-id>/<run-id>.json' runtime/README.md || fail "runtime README must define isolated recommendation run artifact"
...
! grep -Riq --exclude=validate_package.sh '[b]rave' scripts references skills README.md WORKFLOW.md CHANGELOG.md || fail "removed search provider references must stay removed"
! grep -Riq --exclude=validate_package.sh 'hq\.sinajs\.cn\|qt\.gtimg\.cn\|push2\.eastmoney\.com\|push2his\.eastmoney\.com\|q\.stock\.sohu\.com\|api\.tavily\.com\|news\.google\.com/rss' scripts references skills README.md WORKFLOW.md runtime/README.md || fail "unguaranteed web providers must stay removed from runtime policy"
...
grep -q '短线主线优先' skills/a-share-stock-recommendation/SKILL.md || fail "stock recommendation must default to short-term mainline style"
grep -q '板块涨幅、相对强度与成交热度 | 25%' skills/a-share-stock-recommendation/SKILL.md || fail "stock recommendation must prioritize sector speed and heat"
grep -q '主力资金流入与交易结构 | 25%' skills/a-share-stock-recommendation/SKILL.md || fail "stock recommendation must prioritize capital flow"
grep -q 'short_term_fit' skills/a-share-stock-recommendation/SKILL.md || fail "stock recommendation must require short_term_fit"
grep -q 'short_term_mainline' references/run-output-schema.md || fail "run schema must define short_term_mainline policy"
grep -q 'run_session_id' references/run-output-schema.md || fail "run schema must define session isolated runs"
grep -q 'evidence_backed_discovery' references/run-output-schema.md || fail "run schema must define evidence-backed discovery bucket"
grep -q 'autonomy_mode' references/run-output-schema.md || fail "run schema must define bounded autonomy mode"
grep -q 'missing_mainlines' references/run-output-schema.md || fail "run schema must define missing mainline audit"
grep -q '短线主线' references/sector-state.md || fail "sector state ledger must define short-term mainlines"
grep -q '证据型发现' skills/a-share-stock-recommendation/SKILL.md || fail "stock recommendation must allow evidence-backed discovery"
grep -q '系统主动发现' skills/a-share-stock-recommendation/SKILL.md || fail "stock recommendation must define autonomous discovery output"
...
grep -q 'iFinD skill' skills/a-share-capital-flow/SKILL.md || fail "capital flow skill must mention iFinD skill priority"
...
grep -q '按日期和会话隔离' skills/a-share-stock-recommendation/SKILL.md || fail "stock recommendation must generate isolated recommendation run"
grep -q '候选池覆盖' skills/a-share-stock-recommendation/SKILL.md || fail "stock recommendation must audit candidate pool coverage"
grep -q 'selected_mainlines' skills/a-share-stock-recommendation/SKILL.md || fail "stock recommendation must output selected mainlines"
...
grep -q 'rank_mainlines' scripts/generate_candidates.py || fail "candidate generator must rank short-term mainlines"
grep -q 'mainline-count' scripts/generate_candidates.py || fail "candidate generator must expose mainline count"
grep -q 'discovery-count' scripts/generate_candidates.py || fail "candidate generator must expose discovery count"
grep -q 'is_discovery_candidate' scripts/generate_candidates.py || fail "candidate generator must gate discovery candidates"
grep -q 'short_term_fit' scripts/generate_recommendation_run.py || fail "recommendation run generator must preserve short_term_fit"
grep -q 'autonomy' scripts/generate_recommendation_run.py || fail "recommendation run generator must preserve autonomy"
grep -q 'hexin-ifind-stock.get_stock_perfomance' scripts/fetch_capital_flow.py || fail "fetch_capital_flow.py must use iFinD MCP"
